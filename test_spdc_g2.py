...
class TestSPDC(unittest.TestCase):
    ...
    def test_gaus(self):
        ...
        stat, p = shapiro(func.gaussian(xdata,1,0,0.1))#Statistical power 

        self.assertLessEqual(p, 0.05)#Shapiro-Wilk test for Gaussian distribution
        
        with self.assertRaises(ValueError):
            func.gaussian(xdata,0,0,1)#0 coefficient is not allowed
            func.gaussian(xdata,1,0,0)#0 sigma is not allowed
            func.gaussian(xdata,0,0,0)#Both sigma and coefficient
